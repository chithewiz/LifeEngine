...
class GridMap {
    ...
    createIsland(centerCol, centerRow, radius, state) {
        for (let c = centerCol - radius; c <= centerCol + radius; c++) {
            for (let r = centerRow - radius; r <= centerRow + radius; r++) {
                if (this.isValidLoc(c, r)) {
                    let cell = this.cellAt(c, r);
                    if (cell) {
                        cell.isIslandBase = true;
                        const dist = Math.sqrt(Math.pow(c - centerCol, 2) + Math.pow(r - centerRow, 2));
                        if (dist <= radius) {
                            if (Math.random() > 0.2) {
                                this.setCellType(c, r, state);
                            }
                        }
                    } else {
                        console.warn("Null cell at:", c, r);
                    }
                }
            }
        }
